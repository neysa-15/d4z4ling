...
d4z4_features = features_df[features_df["Feature"].str.startswith("d4z4")]
...
def calculate_distal_copy_methylation(summary_df, d4z4_start, d4z4_end):
...
for idx, row in summary_df.iterrows():
    ...
    if row["pLAM_mapped"]:
        ...
        if pd.notna(plam_coords):
            ...
            summary_df = calculate_distal_copy_methylation(summary_df, d4z4_start, d4z4_end)
        ...
        if "distal" in row["ReadLabel"]:
            ...
            read_features_repeats = read_features[read_features["Feature"].str.match(r"d4z4_\d+$")]
            # Then take the last copy of the d4z4 repeat
            # d4z4_{max} if strand is + and d4z4_1 if strand is -
            distal_start, distal_end = get_distal_d4z4_coords(read_features_repeats)

            if distal_start is not None and distal_end is not None:
                summary_df = calculate_distal_copy_methylation(summary_df, distal_start, distal_end)
